...
function Build-WindowsRelease {
    ...
    try {
        # Try different ways to run vpk
        $vpkCommand = $null
        $vpkVersionOutput = $null
        
        # Method 1: Try vpk directly
        try {
            $vpkVersionOutput = & vpk --help 2>&1 | Select-Object -First 1
            $vpkCommand = "vpk"
        }
        catch {
            # Method 2: Find vpk in user profile
            $userProfile = $env:USERPROFILE
            $vpkPath = "$userProfile\.dotnet\tools\vpk.exe"
            if (Test-Path $vpkPath) {
                $vpkVersionOutput = & $vpkPath --help 2>&1 | Select-Object -First 1
                $vpkCommand = $vpkPath
            }
            else {
                throw "VPK not found in any location"
            }
        }
        
        Write-Host "Velopack found: $vpkVersionOutput" -ForegroundColor Green
        ...
        $vpkArgs = @(
            ...
            "--packDir", $publishDirAbs
            "--mainExe", "AkademiTrack.exe"
        )
        
        Write-Host "Running: $vpkCommand $($vpkArgs -join ' ')"
        $vpkOutput = & $vpkCommand @vpkArgs 2>&1
        
        if ($LASTEXITCODE -eq 0) {
            Write-Host "VPK package created successfully" -ForegroundColor Green
            
            # Move VPK files to version folder
            Start-Sleep -Seconds 1
            $vpkReleasesPath = ".\Releases"
            if (Test-Path $vpkReleasesPath) {
                Get-ChildItem $vpkReleasesPath -File | ForEach-Object {
                    $dest = Join-Path $releaseFolder $_.Name
            ...
            Write-Host "VPK pack failed, but continuing..." -ForegroundColor Yellow
            Write-Host $vpkOutput -ForegroundColor Yellow
        }
    }
    catch {
        Write-Host "Velopack (vpk) not found - skipping VPK package creation" -ForegroundColor Yellow
        Write-Host "Install it with: dotnet tool install -g vpk"
    }
